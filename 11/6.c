...
int main() {
    ...
    Pair *requests = (Pair *) malloc(k * sizeof(Pair));
    for (int i = 0; i < k; ++i) {
        scanf("%d %d", &a, &b);
        Pair p; p.first = a - 1; p.second = b - 1;
        requests[i] = p;
    }

    Rebro *e = (Rebro *) malloc((m) * sizeof(Rebro));
    ...
    for (int i = 0; i < m; ++i) {
        ...
        r.a--; r.b--;
        e[i] = r;
//        r1.a = r.b; r1.b = r.a; r1.c = r.c; // todo watch this bro
//        e[2*i+1] = r1;
    }

//    int **dd = (int **) malloc(k * sizeof(int *));
//    int **pp = (int **) malloc(k * sizeof(int *));
    int *d = (int *) malloc(n * sizeof(int));
    int *path = (int *) malloc(n * sizeof(int));
    int *epath = (int *) malloc(n * sizeof(int));

    for (int i = 0; i < k; ++i) { // for all friends
        for (int j = 0; j < n; ++j) d[j] = INF;
        for (int j = 0; j < n; ++j) path[j] = -1;
        for (int j = 0; j < n; ++j) epath[j] = -1;

        d[requests[i].first] = 0;
...
//        dd[i] = d;
        ...
        for (int cur=b; epath[cur] != -1; cur = path[cur])
            cvector_push_back(_path, epath[cur]);

        printf("quarantine  %d %lu  ", d[b], cvector_size(_path));
