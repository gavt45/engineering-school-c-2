...
int main() {
    int n, m, p, k; // вершин, соединялок, запросов мин пути, запросов мин времени
    int a, b, c;
...
//        vector<int> path;
        ...
        for (int cur=b; cur!=-1; cur = pp[a][cur])
            cvector_push_back(path, cur);

        printf("%d  %lu  ", dd[a][b], cvector_size(path));

        for (int j = cvector_size(path) - 1; j >= 0; --j)
            printf("%d ", path[j] + 1);
